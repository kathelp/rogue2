  ...
  describe "GET /setup/:signed_id" do
    ...
    context "with a configured Source (resume)" do
      ...
      it "shows the next due date" do
        ...
        request_record = create(:request, tenant: tenant, source: source, metric_key: "strategy_summary", cadence: "monthly")
        create(:submission_prompt, tenant: tenant, request: request_record, scheduled_for: Time.zone.parse("2026-06-01"))
  ...
  describe "PATCH /setup/:signed_id" do
    context "with submission_method=form" do
      it "returns 302 redirecting to step=done" do
        patch setup_walkthrough_path(signed_id: signed_id),
    ...
    context "with an expired signed_id" do
      it "renders the expired page" do
        token = signed_id  # generate the token NOW so travel_to ages it past expiry
        travel_to(8.days.from_now) do
          patch setup_walkthrough_path(signed_id: token),
                params: { source: { submission_method: "form" } }
          expect(response).to have_http_status(:not_found)
        end
      end
