...
RSpec.describe OnboardingFlow::EscalationCascade do
  ...
  let!(:responsibility) do
    create(:responsibility, tenant: tenant, tenant_question: question,
           primary_contact: contact, fallback_contact_emails: fallback_emails)
  end
  ...
  let(:prompt) do
    create(:submission_prompt, tenant: tenant, request: request_record,
           status: "sent", scheduled_for: scheduled_for, sent_at: scheduled_for)
  end
  ...
  describe ".next_action_for" do
    ...
    context "when due-soon window opens (3 days before period end)" do
      ...
      it "returns nil when due_soon FlowEvent already exists" do
        FlowEvent.record!(event_type: "escalation.due_soon", tenant: tenant,
                          subject: prompt, payload: { severity: "due_soon" })
        result = described_class.next_action_for(prompt: prompt, now: Time.zone.parse("2026-05-29 10:00:00"))
    ...
    context "when overdue window opens (3 days past period end)" do
      before do
        FlowEvent.record!(event_type: "escalation.due_soon", tenant: tenant,
                          subject: prompt, payload: { severity: "due_soon" }, occurred_at: Time.zone.parse("2026-05-29 10:00:00"))
      end
    ...
    context "when fallback fan-out window opens" do
      ...
      before do
        FlowEvent.record!(event_type: "escalation.due_soon", tenant: tenant,
                          subject: prompt, payload: {}, occurred_at: Time.zone.parse("2026-05-29 10:00:00"))
        FlowEvent.record!(event_type: "escalation.overdue", tenant: tenant,
                          subject: prompt, payload: {}, occurred_at: Time.zone.parse("2026-06-03 10:00:00"))
      end
    ...
    context "when GM nudge window opens (after fallbacks exhausted)" do
      ...
      before do
        ...
        FlowEvent.record!(event_type: "escalation.overdue", tenant: tenant, subject: prompt,
                          payload: {}, occurred_at: Time.zone.parse("2026-06-03 10:00:00"))
        FlowEvent.record!(event_type: "escalation.fallback_fanout", tenant: tenant, subject: prompt,
                          payload: { fallback_index: 0, fallback_email: "taylor@smithtoyota.com" },
                          occurred_at: Time.zone.parse("2026-06-07 10:00:00"))
      end
