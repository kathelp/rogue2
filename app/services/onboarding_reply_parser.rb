...
class OnboardingReplyParser
  ...
  ParsedReply = Struct.new(
    # :assign | :self_assign | :skip | :unparseable | :clarification_response
    :intent,
    # String? (normalized lowercase)
    :primary_email,
    # Array<String> (normalized, ordered)
    :fallback_emails,
    # TenantQuestion | nil
    :question,
    # String — meaningful body slice for audit (≤ 4 KB)
    :raw_excerpt,
    # :high | :medium | :low
    :confidence,
    # Array<Symbol>
  ...
  module CcOrdering
    ...
    def self.reorderer?(mail)
      user_agent = (mail.header[:user_agent]&.to_s ||
